    ...
    >
      ...
      >
        <template slot-scope="{row: {fileName}}">
          <span> {{ fileName | valueFilter }} </span>
        </template>
      </el-table-column>
      <el-table-column
        label="文件路径"
      ...
      >
        <template slot-scope="{row}">
          <el-button type="text" icon="el-icon-edit" @click="hanldUpdate(row)" />
          <el-button type="text" icon="el-icon-delete" style="color: #f56c6c" @click="hanldDelete(row)" />
        </template>
      </el-table-column>
    </el-table>
    <pagination
